...
export default async function CategoryDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: category } = await supabase.from('categories').select('*').ilike('name', categoryName).single()
  
  let products: any[] = []
  if (category) {
    const { data: fetchedProducts } = await supabase.from('products').select('*, brands(name)').eq('category_id', category.id)
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 mb-10 flex flex-col md:flex-row items-center gap-8">
        <div className="w-32 h-32 rounded-2xl bg-orange-50 border border-orange-100 flex items-center justify-center p-4">
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <p className="text-gray-500 text-lg mb-4">Browse all wholesale products in this category.</p>
          <div className="flex flex-wrap gap-4 justify-center md:justify-start">
            ...
            <span className="flex items-center gap-1 text-sm bg-orange-50 text-orange-700 px-3 py-1 rounded-full font-medium">
              <Truck className="w-4 h-4" /> Multi-brand Availability
            </span>
          </div>
        </div>
      </div>
          ...
          <div key={product.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-xl transition-shadow">
            ...
            <div className="p-6 flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-bold text-blue-600 tracking-wider uppercase">{product.brands?.name || "Unbranded"}</span>
              ...
              <div className="mt-auto space-y-4">
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-500 mb-6">
                    <div className="flex items-center gap-1"><Truck className="w-4 h-4"/> Next Day</div>
                    <div className="flex items-center gap-1"><ShieldCheck className="w-4 h-4 text-green-500"/> Verified</div>
                  </div>
                </div>
                ...
                {isRetailer ? (
                  <Link href={`/dashboard/products/${product.id}`}>
                    <Button 
                      className={`w-full py-6 rounded-xl font-bold transition-all ${inStock ? 'bg-orange-600 hover:bg-orange-700 text-white shadow-md' : 'bg-gray-100 text-gray-400'}`}
                      disabled={!inStock}
                    >
                      {inStock ? "View Product Details" : "Currently Unavailable"}
                    </Button>
                  </Link>
                ) : (
